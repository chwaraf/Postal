...
local function GetBagItemInfo(bag, slot)
	...
	elseif GetContainerItemInfo then
		local texture, count, locked = select(1, GetContainerItemInfo(bag, slot))
...
function Postal_Express:MAIL_SHOW()
	...
	-- Modern hook (Retail + newer Classic clients)
	if type(HandleModifiedItemClick) == "function" and not self._handleModifiedHooked then
		hooksecurefunc("HandleModifiedItemClick", Postal_Express.HandleModifiedItemClick)
		self._handleModifiedHooked = true
	end
...
function Postal_Express:ContainerFrameItemButtonOnModifiedClick(bag, slot, button)
	...
	if button == "LeftButton"
		and Postal.db.profile.Express.EnableAltClick
		and IsAltKeyDown()
		and SendMailFrame:IsVisible()
		and not CursorHasItem()
	then
		local texture, count = GetBagItemInfo(bag, slot)
	...
	-- Ctrl-click: bulk attach similar items
	elseif button == "LeftButton"
		and Postal.db.profile.Express.BulkSend
	...
	then
		...
		local _, _, itemlocked = GetBagItemInfo(bag, slot)

		local itemq, _, _, itemc, itemsc, _, itemes = select(3, C_Item.GetItemInfo(itemid))
		if not itemq or not itemc then return end
		...
		local bmax = NUM_BAG_FRAMES or 4
		...
		for pass = 0, 4 do
			for b = 0, bmax do
				local numberOfSlots = GetBagNumSlots(b)
				for s = 1, numberOfSlots do
					local tid = GetBagItemID(b, s)
					local _, _, itemlocked2 = GetBagItemInfo(b, s)

					if not tid or itemlocked2 or Postal_Express_IsSoulbound(b, s) then
						-- skip locked, already attached, soulbound
					else
						local tq, _, _, tc, tsc, _, tes = select(3, C_Item.GetItemInfo(tid))
						tsc = (tc or "") .. "." .. (tsc or "")
						tes = tes and #tes > 0

						if (pass == 0 and tid == itemid)
						or (pass == 1 and tsc == itemsc)
						or (pass == 2 and tc == itemc)
						or (pass == 3 and tq == itemq)
						or (pass == 4 and itemq == 0 and tq == 0)
						or (pass == 4 and itemq == 2 and tq == 2 and itemes and tes)
						then
							ClearCursor()
							PickupBagItem(b, s)
							ClickSendMailItemButton()

							local _, _, itemlocked3 = GetBagItemInfo(b, s)
							if itemlocked3 then
								added = added + 1
...
function Postal_Express:ContainerFrameItemButton_OnModifiedClick(this, button, ...)
	...
	if this.GetBagID then
		bag = this:GetBagID()
	else
		bag = this:GetParent() and this:GetParent():GetID()
	end
...
function Postal_Express.HandleModifiedItemClick(itemLink, itemLocation)
	...
	if itemLocation.GetBagAndSlot then
		...
		bag = itemLocation.bagID
		slot = itemLocation.slotIndex
	end
